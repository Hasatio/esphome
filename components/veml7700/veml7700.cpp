...
void VEML7700::dump_config()
{
    ...
    if (this->is_failed())
    {
        ESP_LOGE(TAG, "Communication with I2C VEML7700 failed!");
    }
} 
void VEML7700::loop()
{
        ...
        switch (this->state_)
        {
            ...
            case State::DATA_COLLECTED:
                ...
                else
                {
                    ...
                    if (err == i2c::ERROR_OK)
                    {
                        this->set_timeout(1 * get_itime_ms(this->readings_.actual_time), [this]() { this->state_ = State::READY_TO_APPLY_ADJUSTMENTS; });
                    }
                    else
                    {
            ...
            case State::READY_TO_APPLY_ADJUSTMENTS:
                ...
                if (err == i2c::ERROR_OK)
                {
                    this->set_timeout(3 * get_itime_ms(this->readings_.actual_time), [this]() { this->state_ = State::COLLECTING_DATA; });
                }
                else
                {
                    this->state_ = State::IDLE;
...
ErrorCode VEML7700Component::read_sensor_output_(Readings &data)
{
    ...
    auto white_err = this->read_register((uint8_t) CommandRegisters::WHITE, (uint8_t *) &data.white_counts, VEML_REG_SIZE, false);
    ...
    auto err = this->read_register((uint8_t) CommandRegisters::ALS_CONF_0, (uint8_t *) conf.raw_bytes, VEML_REG_SIZE, false);
    if (err != i2c::ERROR_OK)
    {
    ...
    ESP_LOGV(TAG, "Data from sensors: ALS = %d, WHITE = %d, Gain = %s, Time = %d ms", data.als_counts, data.white_counts, get_gain_str(data.actual_gain), get_itime_ms(data.actual_time));
...
void VEML7700Component::apply_lux_calculation_(Readings &data)
{
    ...
    float lux_resolution = (MAX_ITIME_MS / (float) get_itime_ms(data.actual_time)) * (MAX_GAIN / get_gain_coeff(data.actual_gain)) * MAX_LX_RESOLUTION;
    ESP_LOGV(TAG, "Lux resolution for (%d, %s) = %.4f ", get_itime_ms(data.actual_time), get_gain_str(data.actual_gain), lux_resolution);
