    ...
    for (size_t i = 0; i < 6; i++)
    {
        if (type[i] == 1)
        {
            ...
            if (stat[i] == 1)
            {
                ...
                tot[i][0] += static_cast<uint16_t>(tot[i][1] + (dose[i] > 0 ? calib[i] : 0) * mint * 10) / 100000;
                tot[i][1] = static_cast<uint16_t>(tot[i][1] + (dose[i] > 0 ? calib[i] : 0) * mint * 10) % 100000;
...
void Pump::Circulation_Controller(uint8_t stat[])
{
    ...
    for (size_t i = 0; i < 6; i++)
    {
        if (type[i] == 2)
        {
            ...
            {
                //  std::cout << "Pump_stat[" << i << "] = " << (stat[i] ? "true" : "false") << "\n";
                tot[i][0] += static_cast<uint16_t>(tot[i][1] + (circ[i] > 0 ? calib[i] : 0) * mint * 10) / 100000;
                tot[i][1] = static_cast<uint16_t>(tot[i][1] + (circ[i] > 0 ? calib[i] : 0) * mint * 10) % 100000;
