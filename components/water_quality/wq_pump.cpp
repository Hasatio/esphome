...
void Pump::Circulation_Controller(float pump[])
{
    ...
    for (size_t i = 0; i < 6; i++)
    {
        if (type[i] == 2)
        {
            ...
            {
                if (!get_Calibration_Mode())
                    tot[i][1] = static_cast<uint32_t>(tot[i][1] + (circ[i] > 0 ? calib[i] : 0) * min * 1) % 1000;
                tot[i][0] += static_cast<uint32_t>(tot[i][1] + (circ[i] > 0 ? calib[i] : 0) * min * 1) / 1000;
                ESP_LOGI(TAG,"min = %f", min);
                ESP_LOGI(TAG,"total = %f", static_cast<uint32_t>(tot[i][1] + (circ[i] > 0 ? calib[i] : 0) * min) / 1000);
