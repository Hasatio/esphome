...
void Analog::Analog_Input_Driver(float volts[])
{
    ...
    set_VPow_Val((float)volts[3] * 6); //Vin = Vout * (R1 + R2) / R2; R1 = 10k, R2 = 2k

    // ESP_LOGD(TAG,"VPow = %f", get_VPow_Val());
    
	float lvl[2];
    lvl[0] = (float)volts[1] * 100 / 5 * AnInLvl_ResMax[0] / (1000 + AnInLvl_ResMax[0]) - 5 * AnInLvl_ResMin[0] / (1000 + AnInLvl_ResMin[0]); //Vout = Vin * R2 / (R1 + R2); R1 = 10k
    lvl[1] = (float)volts[2] * 100 / 5 * AnInLvl_ResMax[1] / (1000 + AnInLvl_ResMax[1]) - 5 * AnInLvl_ResMin[1] / (1000 + AnInLvl_ResMin[1]); //Vout = Vin * R2 / (R1 + R2); R1 = 10k
    set_Lvl_Perc(lvl);
